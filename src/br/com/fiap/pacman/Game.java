...
public class Game extends JFrame implements KeyListener {
    ...
    private int speed = 5;
    private int turnsOfInvincibility = 0;
    ...
    private void run() {
        while (player.getLife() > 0) {
            if (player.isInvincible()) turnsOfInvincibility--;
            if (turnsOfInvincibility == 0) player.setInvincible(false);
            ...
            if (player.isColiding(bomb)) {
                player.loseLife();
                bomb.setVisible(false);
            }
            if (player.isColiding(ghost1)) player.loseLife();
            if (player.isColiding(ghost2)) player.loseLife();
            if (player.isColiding(ghost3)) player.loseLife();
            if (player.isColiding(ghost4)) player.loseLife();

            if (player.isColiding(booster)) {
                turnsOfInvincibility = booster.getInvincibleDuration();
